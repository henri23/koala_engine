...
KOALA_API b8 filesystem_open(
    const char* path,
    File_Modes mode,
    b8 binary,
    File_Handle* out_handle);

KOALA_API void filesystem_close(File_Handle* handle);

// Used for text files, reads an entire line of text until it finds the \n or
// EOF characters
KOALA_API b8 filesystem_read_line(
    File_Handle* handle,
    char** line_buf);

KOALA_API b8 filesystem_write_line(
    File_Handle* handle,
    const char* text);
...
KOALA_API b8 filesystem_read_all_bytes(
    File_Handle* handle,
    u8** out_bytes,
    u64* out_bytes_read);

KOALA_API b8 filesystem_write(
    File_Handle* handle,
    u64 data_size,
    const void* data,
