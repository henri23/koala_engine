...
b8 vulkan_graphics_pipeline_create(
    ...
    // Dynamic state. Usually the graphics pipeline is immutable and to change
    // state entities, we need to recreate the pipeline, however there is the
    // so-called dynamic state, which specifies which components of the pipeline
    // can be mutable in runtime, w/o needing to recreate the whole pipeline
    ...
    ENGINE_INFO("Graphics pipeline layout created.");
    ...
    ENGINE_ERROR(
        "vkCreateGraphicsPipelines failed with %s.",
